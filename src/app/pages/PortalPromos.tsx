...
interface LivePromo  { id: number; title: string; description: string | null; promotion_img_url?: string | null }
interface PromoForm  { title: string; description: string; image_url: string }
const EMPTY: PromoForm = { title: '', description: '', image_url: '' }

export default function PortalPromos() {
  ...
  const loadData = async (id: number) => {
    setLoading(true)
    const [promRes, pendRes] = await Promise.all([
      supabase.from('promotion').select('id, title, description, promotion_img_url').eq('site_id', id).eq('active', true).order('id'),
      supabase.from('business_edit_request').select('payload').eq('site_id', id).eq('type', 'promo').eq('status', 'pending'),
    ])
    setPromos(promRes.data ?? [])
    setPendingIds(new Set((pendRes.data ?? []).map((r: any) => r.payload?.promo_id).filter(Boolean)))
    setLoading(false)
  }

  useEffect(() => { if (siteId) { setSuccess(null); loadData(siteId) } }, [siteId])

  const setF = (k: keyof PromoForm, v: string) => setForm(prev => ({ ...prev, [k]: v }))
  const openAdd  = () => { setForm(EMPTY); setEditId(null); setShowForm(true); setSuccess(null); setError(null) }
  const openEdit = (p: LivePromo) => {
    setForm({ title: p.title, description: p.description ?? '', image_url: p.promotion_img_url ?? '' })
    setEditId(p.id); setShowForm(true); setSuccess(null); setError(null)
  }
  ...
  const handleImageUpload = async (file: File) => {
    ...
    const path = `${authId}/promo-edit-${siteId}-${Date.now()}.${ext}`
    const { error } = await supabase.storage.from('business-registrations').upload(path, file, { upsert: true })
    if (!error) { const { data } = supabase.storage.from('business-registrations').getPublicUrl(path); setF('image_url', data.publicUrl) }
    setUploading(false)
  }
  ...
  const handleSubmit = async () => {
    ...
    const { error: insErr } = await supabase.from('business_edit_request').insert({
      site_id: siteId, auth_id: authId, type: 'promo',
      action: editId ? 'update' : 'create',
      payload: { ...form, ...(editId ? { promo_id: editId } : {}) },
    })
    setSubmitting(false)
    if (insErr) { setError('Error al enviar. Intenta de nuevo.'); return }
    setSuccess(editId ? 'Cambio enviado a revisión.' : 'Promoción enviada a revisión.')
    setShowForm(false)
  }
  ...
  const handleDelete = async (promoId: number) => {
    if (!authId || !siteId) return
    const { error: insErr } = await supabase.from('business_edit_request').insert({
      site_id: siteId, auth_id: authId, type: 'promo', action: 'delete', payload: { promo_id: promoId },
    })
    if (!insErr) { setSuccess('Solicitud de eliminación enviada.'); setPendingIds(prev => new Set([...prev, promoId])) }
    setConfirmDel(null)
  }
  ...
  return (
    <div className="min-h-screen bg-[#F5F7F9] flex flex-col">
      <PortalHeader
        ...
        backTo={backTo}
        gradientClass="bg-gradient-to-r from-[#B8860B] to-[#8B6914]"
        sites={sites}
        selectedSiteId={siteId}
        onSiteChange={id => navigate(`/portal/promos?site=${id}`)}
      />

      <div className="flex-1 max-w-lg mx-auto w-full px-4 py-6 space-y-4">
        <div className="flex justify-end">
          <button onClick={openAdd} className="flex items-center gap-1.5 bg-[#B8860B] hover:bg-[#9A700A] text-white text-sm font-semibold px-4 py-2 rounded-xl transition-all">
        ...
        {showForm && (
          <div className="bg-white rounded-2xl border border-[#25B3CC]/30 shadow-sm p-5 space-y-4">
            <div className="flex items-center justify-between">
              <h3 className="font-bold text-gray-800">{editId ? 'Editar promoción' : 'Nueva promoción'}</h3>
            ...
            <div>
              ...
              <input className={inputCls} value={form.title} onChange={e => setF('title', e.target.value)} maxLength={100} />
            </div>
            <div>
            ...
            <div>
              ...
              {form.image_url ? (
                <div className="flex items-center gap-3">
                  <img src={form.image_url} alt="promo" className="w-16 h-16 rounded-xl object-cover border border-gray-200" />
                  <label className="cursor-pointer text-[#25B3CC] text-sm font-medium hover:text-[#1E9DB5]">
                    Cambiar imagen
        ...
        {loading ? (
          ...
          <div className="text-center py-12 text-gray-400">
            <p className="text-sm">No tienes promociones activas.</p>
            <button onClick={openAdd} className="mt-3 text-[#25B3CC] text-sm font-medium hover:text-[#1E9DB5]">+ Agregar promoción</button>
          </div>
        ) : (
          promos.map(p => (
            <div key={p.id} className="bg-white rounded-2xl border border-gray-100 p-4 flex items-start justify-between gap-3">
              <div className="flex items-start gap-3 flex-1 min-w-0">
                {p.promotion_img_url && <img src={p.promotion_img_url} alt={p.title} className="w-12 h-12 rounded-xl object-cover flex-shrink-0 border border-gray-100" />}
                <div className="min-w-0">
                  <p className="font-semibold text-gray-900 text-sm">{p.title}</p>
                  {p.description && <p className="text-gray-400 text-xs mt-0.5 line-clamp-2">{p.description}</p>}
                  {pendingIds.has(p.id) && <div className="flex items-center gap-1 mt-1.5"><Clock className="w-3 h-3 text-yellow-500" /><span className="text-yellow-600 text-xs">Cambio en revisión</span></div>}
                </div>
              </div>
              {confirmDel === p.id ? (
                <div className="flex items-center gap-2 flex-shrink-0">
                  <span className="text-xs text-gray-600">¿Eliminar?</span>
                  <button onClick={() => handleDelete(p.id)} className="text-xs text-red-600 font-semibold">Sí</button>
                  <button onClick={() => setConfirmDel(null)} className="text-xs text-gray-500">No</button>
                </div>
              ) : (
                <div className="flex items-center gap-2 flex-shrink-0">
                  <button onClick={() => openEdit(p)} className="p-1.5 text-gray-400 hover:text-[#25B3CC] transition-colors"><Pencil className="w-4 h-4" /></button>
                  <button onClick={() => setConfirmDel(p.id)} className="p-1.5 text-gray-400 hover:text-red-500 transition-colors"><Trash2 className="w-4 h-4" /></button>
                </div>
              )}
            </div>
          ))
