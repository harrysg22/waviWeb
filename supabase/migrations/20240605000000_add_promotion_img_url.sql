...
BEGIN
    ...
    FOR v_company_id IN SELECT id FROM company WHERE account_id = v_account_id LOOP
      FOR v_site_id IN SELECT id FROM site WHERE company_id = v_company_id LOOP
        DELETE FROM service_image
          WHERE service_id IN (SELECT id FROM service WHERE site_id = v_site_id);
        DELETE FROM service WHERE site_id = v_site_id;
        DELETE FROM event_image
          WHERE event_id IN (SELECT id FROM event WHERE site_id = v_site_id);
        DELETE FROM event WHERE site_id = v_site_id;
        DELETE FROM business_hours WHERE site_id = v_site_id;
        DELETE FROM site_cuisine WHERE site_id = v_site_id;
        DELETE FROM site_additional_services WHERE site_id = v_site_id;
        DELETE FROM promotion_image WHERE promotion_id IN (SELECT id FROM promotion WHERE site_id = v_site_id);
        DELETE FROM promotion WHERE site_id = v_site_id;
        DELETE FROM site_image WHERE site_id = v_site_id;
        DELETE FROM site WHERE id = v_site_id;
      END LOOP;
      DELETE FROM company_category WHERE company_id = v_company_id;
      DELETE FROM company_contact WHERE company_id = v_company_id;
      DELETE FROM image WHERE company_id = v_company_id;
      DELETE FROM company WHERE id = v_company_id;
    END LOOP;
  END IF;

  -- 1. Crear company
  INSERT INTO company (name, description, account_id)
  VALUES (v_reg.business_name, COALESCE(v_reg.description, ''), v_account_id)
  ...
  INSERT INTO site (
    ...
    v_reg.zone_id,
    true,
    COALESCE(NULLIF(v_reg.logo_url, ''),
             CASE WHEN array_length(v_reg.image_urls, 1) > 0 THEN v_reg.image_urls[1] ELSE NULL END)
  )
  ...
  FOR v_hour IN SELECT * FROM jsonb_array_elements(v_reg.business_hours) LOOP
    ...
    VALUES (
      v_site_id,
      (v_hour->>'weekday')::INT,
      (v_hour->>'start_time')::TIME,
      (v_hour->>'end_time')::TIME
    );
  END LOOP;
  ...
  FOR v_ev IN SELECT * FROM jsonb_array_elements(COALESCE(v_reg.events, '[]'::jsonb)) LOOP
    INSERT INTO event (
      ...
      v_ev->>'titulo',
      v_ev->>'descripcion',
      NULLIF(NULLIF(trim(v_ev->>'fecha_inicio'),'') || ' ' || COALESCE(NULLIF(trim(v_ev->>'hora'),''), '00:00'), ' ')::TIMESTAMP,
      NULLIF(NULLIF(trim(v_ev->>'fecha_fin'),'') || ' ' || COALESCE(NULLIF(trim(v_ev->>'hora'),''), '00:00'), ' ')::TIMESTAMP,
